...
from app.schemas.sync_run import BulkSyncRequest, BulkSyncResponse, SyncRunResponse
...
def sync_all_instances_route(
    payload: BulkSyncRequest | None = Body(default=None),
    db: Session = Depends(get_db),
) -> BulkSyncResponse:
    """Run manual sync against all enabled instances or a selected enabled subset."""
    try:
        return sync_all_instances(db, instance_ids=payload.instance_ids if payload else None)
    except OperationalError as exc:
        _raise_if_sqlite_locked(exc, db)
